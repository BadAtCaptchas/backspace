...
export async function authRoutes(app: FastifyInstance): Promise<void> {
  app.post<{ Body: RegisterRequest }>('/api/auth/register', {
    ...
    // Public registration must never mutate an existing federation-replicated
    // stub. The caller-provided homeInstance/homeUserId values are not proof of
    // control of that remote identity; turning a stub into a login-capable
    // account here would grant access to that stub's existing memberships.
