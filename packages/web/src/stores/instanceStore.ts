...
interface CachedInstanceToken {
  ...
  /** Per-remote credential generated by this client. Never reuse the home password for remote auth. */
  remoteSecret?: string;
}
...
function saveCachedTokens(instances: ConnectedInstance[], userId: string, pendingSyncFlags?: Record<string, boolean>): void {
  ...
  for (const inst of instances) {
    ...
    cache[inst.origin] = {
      ...
      remoteSecret: existing[inst.origin]?.remoteSecret,
    };
  }
  localStorage.setItem(storageKey(userId), JSON.stringify(cache));
}

function generateRemoteSecret(): string {
...
export const useInstanceStore = create<InstanceState>((set, get) => ({
  ...
  connectToRemote: async (origin: string, password: string, displayName?: string) => {
    ...
    try {
      ...
      set((state) => {
        ...
        if (!targetIsHome) {
          const cache = loadCachedTokens(currentUser.id);
          cache[origin] = {
            token: response.token,
  ...
  reauthenticateInstance: async (origin: string, password: string) => {
    ...
    // Clear pending password sync after a successful re-authentication.
